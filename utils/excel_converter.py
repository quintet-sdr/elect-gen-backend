...
def get_excel_distribution(elective):
    ...
    for distribution_name, distribution_data in data.items():
        ...
        df["picked priority"] = [
            students_priorities[row["student"]][row["course"]]
...
def get_excel_current_hum():
    ...
    constraints = pd.read_sql_query(
        "SELECT id, course_codename, student_email FROM constraints_hum", db.bind
    )
...
def get_excel_current_tech():
    ...
    constraints = pd.read_sql_query(
        "SELECT id, course_codename, student_email FROM constraints_tech", db.bind
    )
    for i, row in courses.iterrows():
        row["groups"] = ";".join(row["groups"])
        courses.at[i, "groups"] = row["groups"]
...
def get_excel_example_hum():
    ...
    constrains = pd.DataFrame(
        {"course_codename": ["EXAMPLE"], "student_email": ["a.b@innopolis.university"]}
    )
